...
public class old_FlockEntity : MonoBehaviour
{
private float speed = 1f;
    ...
    private void Update()
    {
        if (Vector3.Distance(transform.position, player.transform.position) >= old_GlobalFlock.AirSize)
        {
            this.turning = true;
        }
        ...
        if (this.turning)
        {
            Vector3 direction = player.transform.position + new Vector3(0, Random.Range(10f, old_GlobalFlock.AirSize)) -transform.position;
        ...
        {
            //good stuff happens!!!!
            old_FlockEntity[] birds = old_GlobalFlock.FlockEntities;
            ...
            float gSpeed = 0.5f;
            Vector3 goalPos = old_GlobalFlock.instance.GoalPos;

            float dist;
            int groupSize = 0;

            foreach (old_FlockEntity bird in birds)
            {
                if (bird != this)
                {
