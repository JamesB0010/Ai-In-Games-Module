...
using Random = UnityEngine.Random;
    ...
    public struct FlockEntityJob : IJobParallelForTransform
    {
        ...
        public void Execute(int index, TransformAccess transform)
        {
            ...
            if (Vector3.Distance(transform.position, playerPositionVec) >= MultiThreadedGlobalFlock.AirSize)
            {
                this.turning = true;
            ...
            if (this.turning)
            {
                Vector3 direction = playerPositionVec + new Vector3(0, this.random.NextFloat(10f, MultiThreadedGlobalFlock.AirSize), 0) - transform.position;
